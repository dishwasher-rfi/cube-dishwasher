...
from numpy import clip as sp_threshold
...
class FlagAlgorithm(object):
    ...
    * compute - detect RFI (TO BE OVERRIDDEN)
    ...
    def get_def_params_names(cls):
        ...
        """
        #return list(cls.get_def_params().keys())
        return 'pippe'
    ...
    def get_def_params_values(cls):
        ...
        """
        return list(cls.get_def_params().values())
    ...
    def get_output_labels(self):
        ...
        """
        return list(self.get_output().keys())
    ...
    def is_out_selected(self, label):
        ...
        """
        return label in self.get_output()
    ...
    def is_out_avail(self, label):
        ...
        label - output option label
        """
        return label in dict(list(zip(self.get_avail_output(),self.get_avail_output())))
    ...
    def is_out_default(self, label):
        ...
        label - output option label
        """
        return label in dict(list(zip(self.get_def_output(),self.get_def_output())))
    ...
    * not_normal:
    ...
    def compute(self):
        ...
        if self.num_of_rms_above_median[0] <= self.num_of_rms_above_median[1] or self.num_of_rms_above_median[0] >= self.num_of_rms_above_median[2]:
            raise ValueError
            return 0
        for lab in list(self.flag_results.keys()):
            ...
            #res = sp_threshold(self.data[m_ind[lab]]-med, self.num_of_rms_above_median[0]*rms, None)
            res = sp_threshold(self.data[m_ind[lab]]-self.num_of_rms_above_median[0]*rms, 0, 1)
            res[res > 0] = 1
...
class FullChan(FlagAlgorithm):
    ...
    * not_normal:
    ...
    #    th = np.percentile(data1, th_prctile)
        thl = []
        for ii in range(data1.shape[0]-9):
...
#           self.upd_param_value('level',int(np_ceil(np_log2(self.data.shape[0])).astype(int)))
    ...
    def _define_output(self):
        ...
        self._set_avail_output(list(map(str,list(range(self.level)))))
        self._set_def_output(list(map(str,list(range(self.level/2, min(self.level, self.level/2+3))))))
...
#        w = pywt.Wavelet('haar')
    ...
    def noise_dwt(cls, coeff, w):
        ...
        std_l.append(stdv)
        for ll in range(len(coeff)-2,0,-1):
            stdv = stdv*k_std
            std_l.append(stdv)
        std_l.append(0)
    ...
    def waverec(cls, coeff, w, level, offset = False):
        ...
        loc_coeff = coeff[:]
        for ll in range(level+1, len(coeff)):
