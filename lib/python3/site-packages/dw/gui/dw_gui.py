...
#        self.set_flag_toolbar()
    ...
    def set_menu(self):
        ...
        open_action = create_action(self, _("Open files.."),
                                    shortcut="Ctrl+O",
                                    icon=get_icon('fileopen.png'),
                                    tip=_("Open data files"),
                                    triggered=self.open_files)
        cube_action = create_action(self, _("Open cube..."),
                                    shortcut="Ctrl+B",
                                    icon=get_icon('fileopen.png'),
                                    tip=_("Open a data cube"),
                                    triggered=self.open_cube)
        ...
        add_actions(file_menu, ( open_action, cube_action, None, close_action, None, quit_action))
...
#                                deflag_action, merge_flag_action,
                                del_flag_action, flag_widget_action, None,
                                propag_flag_action,
                                propag_feeds_action,
                                None, auto_flag_action))
        file_next = create_action(self, _("Next file..."),
                                   icon=get_icon(self.icon_path+'next.png'),
                                   tip=_("Next file"),
                                   triggered=self.file_next)
        file_previous = create_action(self, _("Previous file..."),
                                   icon=get_icon(self.icon_path+'previous.png'),
                                   tip=_("Previous file"),
                                   triggered=self.file_previous)
        file_info = create_action(self, _("File info..."),
                                   icon=get_icon(self.icon_path+'info.png'),
                                   tip=_("File info"),
        ...
        add_actions(main_toolbar, ( open_action, None, file_previous, file_next,))
...
#------I/O
    def file_previous(self):
        ...
        try:
           if self.data.file_mode == 'single':
              return
           nf = len(self.data.file_list)
           nc = self.data.file_index - 1
           if nc < 0:
              nc = nf - 1
           file_index = nc
           file_list = self.data.file_list
           file_type = self.data.file_type
           file_mode = self.data.file_mode
           ret = self.close_file()
           if ret == -1:
              return
           self.build_data(file_list, file_type, file_index, file_mode)
        except:
           pass
    ...
    def file_info(self):
        """Show informations on file active"""
        try:
           nc = self.data.file_index
           msgBox = QMessageBox()
           msgBox.setText('Cube current file '+str(nc)+': \n'+basename(self.data.file_list[nc]))
           msgBox.exec_()
        except:
           pass
    ...
    def open_files(self):
        ...
        flret, choosentype = QFileDialog.getOpenFileNames(self,
                "Select file list", "~/",
                "Data Files (*.fits *.hdf5 *.hdf *.h5);;All files (*)")         
        ...
        if flret:
           ...
           filelist = []
           for f in flret:
               if str.upper(basename(f)[:3]) == 'SUM':
                  nf = nf - 1
                  continue
               extension = splitext(str(f))[1]
               if extension not in ['.fits', '.hdf5', '.hdf', '.h5']:
                  msgBox = QMessageBox()
                  msgBox.setText("Wrong file type: "+basename(str(f)))
                  msgBox.exec_()
                  return -1
           ...
           if nf > 1:
              ...
              filename = filelist
               ...
               msgBox = QMessageBox()
               msgBox.setText(basename(str(f)+' is a summary file'))
               msgBox.exec_()
               return -1
           else:
              file_mode = 'single'
              filename = flret[0]
           file_index = 0
           self.build_data(filename, file_type, file_index, file_mode)

    def build_data(self, filename, file_type, file_index, file_mode):
            self.data = dwdata.DWData()
            self.data.open_data(filename, file_mode, file_index, file_type)
            if file_type == 'fits':
<<<<<<< HEAD
                if self.data.data_type_dict[0][0] == 'spectra':
=======
                if self.data.data_type_dict[0][0] == 'spectra':#deve essere 'spectra'
>>>>>>> a4737cc80d8732f4eae475ae1f4f29b455566c91
                    widg_type = 0
            ...
            if self.data.file_mode == 'multiple':
               title = 'File ('+str(file_index + 1)+'/'+str(len(filename))+') '+basename(filename[file_index])
            elif self.data.file_mode == 'cube':
               title = 'Cube ('+str(file_index + 1)+'/'+str(len(filename))+') '+basename(filename[file_index])
            elif self.data.file_mode == 'single':
               title = basename(filename) 
            self.setWindowTitle(APP_NAME+" - "+title)
...
#            self.mainwidget.refresh_list()
    ...
    def open_cube(self):
        ...
        sys.stdout = None
        self.close_file()
        dirname = QFileDialog.getExistingDirectory(self, _("Open"), "",
                                       options=QFileDialog.ShowDirsOnly)
        sys.stdin, sys.stdout, sys.stderr = saved_in, saved_out, saved_err
        if dirname:
           filename = []
           extension = None
           for path in scandir(dirname):
              if path.is_file():
                 if str.upper(basename(path)[:3]) == 'SUM':
                    continue
                 extension = splitext(str(path.name))[1]
                 if extension not in ['.fits', '.hdf5', '.hdf', '.h5']:
                    msgBox = QMessageBox()
                    msgBox.setText("Wrong file type: "+path.name)
                    msgBox.exec_()
                    return -1
                 filename.append(dirname+'/'+path.name)
           if len(filename) == 0:
              msgBox = QMessageBox()
              msgBox.setText("Wrong files type")
              msgBox.exec_()
              return -1
           list.sort(filename)
    ...
    def open_file(self):
        ...
        try:
            for i in self.mainwidget.imagewidget.plot.items:
                if dwwidg.is_flag(i) and (i.title().text() != 'Average cross section' and  i.title().text() != 'Void'):
                    msgBox = QMessageBox()
    ...
    def del_selected_flag_image_item(self):
        try:
           self.mainwidget.del_selected_flag_image_item(self.data.file_list)
        except:
           self.button_disabled()
           pass
    ...
    def button_disabled(self):
        status = self.statusBar()
        status.showMessage(_("Command active after file opening"), 7000)
