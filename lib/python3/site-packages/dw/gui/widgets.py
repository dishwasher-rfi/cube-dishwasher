...
from PyQt5.QtWidgets  import QWidget, QMessageBox, QSplitter, QListWidget, QPushButton, QVBoxLayout, QGridLayout, QFileDialog
from PyQt5.QtCore import QSize, QT_VERSION_STR, PYQT_VERSION_STR, Qt, QVariant
#from guidata.qt.compat import getopenfilename, getopenfilenames
from guidata.dataset.datatypes import DataSet, GetAttrProp
from guidata.dataset.dataitems import (IntItem, FloatItem, FloatArrayItem, StringItem)
...
from guidata.dataset.datatypes import BeginGroup, EndGroup, ValueProp
...
class AlgSelectionWidget(DataSet):
    ...
    def __init__(self, rfi_alg_dict):
        ...
        choices = []
        for key in list(rfi_alg_dict.keys()):
...
class AlgParamWidget(DataSet):
    ...
    def info_values(self, dw_data, row):
        ...
        for i in datas:
            ...
            if math.isnan(M) or math.isnan(m) or math.isnan(r):
               msgBox = QMessageBox()
               msgBox.setText("NaN value encountered in scalar divide")
               msgBox.exec_()
               return -1
            ret.append((M, m, r))
        return ret
...
#        self.imagewidget.plot.SIG_ITEMS_CHANGED.connect(self.item_notsaved)
        self.imagewidget.plot.SIG_ITEMS_CHANGED.connect(self.upd_flag_items)
...
#        add_actions(toolbar, [flag_selected])
    ...
    def refresh_list(self):
        ...
        #print(('data_ref '+ str(self.data_ref.datasets)))

    def log_toggle(self):
        """Toggle logaritmic scale on/off in the x cross section plot"""
        if not self.get_cross_items():
           msgBox = QMessageBox()
           msgBox.setText('Use Cross section or Average cross section first!')
           msgBox.exec_()
           return
        else:
    ...
    def mu_toggle(self):
        ...
        if self.item is not None:
            if to_phys_units:
                ...
                self.item.set_xy(np_array(list(range(len(x)))),np_array(list(range(len(y)))))
                flag_image_mu_toggle(self.get_flag_image_items(),
                                     np_array(list(range(len(x)))),
                                     np_array(list(range(len(y)))))
    ...
    def get_cross_items(self):
        """Return Cross Section and Average Cross Section Selected
        """
        items = False
        for item in self.imagewidget.plot.items:
            if item.title().text() == 'Cross section' or item.title().text() == 'Average cross section' :
                  items = True
        return items
...
#        self.imagewidget.plot.replot()
    ...
    def upd_metadata(self):
        ...
        try:
            ...
            self.image.name = str(self.image.a_source[y], 'UTF-8')
...
#            if item.title().text() == 'Flag':
            if item.title().text() == 'Flag' or (type(item) == XRangeSelection and item.title().text() == ''):
                try:
        ...
        if isinstance(data, list):
            ...
            for dat in data:
                if phys_units:
                    ...
                    self.item = make.xyimage(np_array(list(range(len(x)))),np_array(list(range(len(y)))),dat,title=title[ii])
            ...
            if self.item is not None:
                ...
                if phys_units:
                    ...
                    self.axes_phys_unit = False
                    self.item = make.xyimage(np_array(list(range(len(x)))),np_array(list(range(len(y)))),data,title=title)
                    self.imagewidget.plot.set_titles(xlabel=prefs.X_LABEL_ARBITRARY, ylabel=prefs.Y_LABEL_ARBITRARY)
                self.item.interpolate=(0,)
    ...
    def checked_items_to_flag(self):
        ...
        for item in items:
            if item.title().text() == 'Average cross section' or item.title().text() == 'Void':
               msgBox = QMessageBox()
               msgBox.setText("No flag items to process!")
               ret = msgBox.exec_()
               return -1
...
#        self.imagewidget.plot.replot()
    ...
    def show_flagging_sets(self):
        ...
        for k_flagset, flagset in list(flagsets.items()):
    ...
    def del_selected_flag_image_item(self, filelist):
        ...
        msgBox.addButton(QMessageBox.Cancel)
        if self.data_ref.file_mode == 'single':
           msgBox.setInformativeText("Press OK to continue")
           msgBox.addButton(QMessageBox.Ok)
        else:
           msgBox.setInformativeText("Press Current or All to continue")
           msgBox.addButton(QPushButton('Current'), QMessageBox.NoRole)
           msgBox.addButton(QPushButton('All'), QMessageBox.YesRole)
        msgBox.setDefaultButton(QMessageBox.Cancel)
        ret = msgBox.exec_()
        ...
        for item in items:
            self.data_ref.del_flagset(row, item.dw_k_flagset, filelist[0])

        if ret:
           for item in saveitems:
               self.data_ref.del_flagset(row, item.dw_k_flagset, filelist)
    ...
    def propag_flag_table(self):
        ...
        if self.data_ref.file_mode == 'multiple' or self.data_ref.file_mode == 'cube':
           msgBox = QMessageBox()
           msgBox.setText("Append the current flag table to the files")
           msgBox.setInformativeText("Press Select or All to continue")
           msgBox.addButton(QMessageBox.Cancel)
           msgBox.addButton(QPushButton('Select'), QMessageBox.NoRole)
           msgBox.addButton(QPushButton('All'), QMessageBox.YesRole)
           msgBox.setDefaultButton(QMessageBox.Cancel)
           ret = msgBox.exec_()
           if ret == QMessageBox.Cancel:
        ...
        if ret == False:
           flret = QFileDialog.getOpenFileNames(self,
               _("Select file list"),
               dirname(self.data_ref.file_name))[0]
           if len(flret) == 0:
               return -1
           ...
           filelist = self.data_ref.file_list

        msgBox = QMessageBox()
        msgBox.setText("Confirm the append flag?")
        msgBox.addButton(QMessageBox.Cancel)
        msgBox.addButton(QMessageBox.Ok)
        msgBox.setDefaultButton(QMessageBox.Cancel)
        ret = msgBox.exec_()
    ...
    def propag_feeds(self):
        ...
        for key in list(self.mem_dict.keys()):
...
#            self.update_rect_flag_item(xmin, ymin, xmax, ymax)
    ...
    def rfi_dect_action(self):
        ...
        if ret:
            if alg_selection.choice in self.data_ref.dect_alg:
                row = self.tablelist.currentRow()
...
#            print((type(items[0].title().text().title()))
            for k in list(self.data_ref.get_flagset_meta(row, items[0].dw_k_flagset).keys()):
            ...
            if len(items) == 1:
                try:
                    ...
                    for k in ['flagresult','params','algorithm']:
                        if k == 'params':
                            value = ''
                            for i in list(getattr(items[0], k).keys()):
...
#  ft  def active_item_changed(self):
    ...
    def open_corr_file(self):
        ...
        corr_file = QFileDialog.getOpenFileName(
           self, "Select file list",
           dirname(self.data_ref.file_name))[0]
        if corr_file == None:
            return -1
    ...
    def item_notsaved(self): #TODO: Items turn to black when crosssection is uded. It shouldn't be
        plotlist = self.imagewidget.itemlist.listwidget
        items = plotlist.items
        for i in items:
            if i.title().text() == 'Average cross section':
                rawitem = plotlist.item(items.index(i))
                rawitem.setForeground(QColor('black'))
            elif is_flag(i):
                rawitem = plotlist.item(items.index(i))
                rawitem.setForeground(QColor('red'))
                if i.title().text() == 'Flag' or (type(i) == XRangeSelection and i.title().text() == ''):
                   #print(('Flag'))
                   title, rif = self.gen_flag_name_rif(i)
                   rawitem.setText(title)
...
#            data = data.transpose()[::-1].transpose()
        ...
        for data in data_l:
            if phys_units:
                ...
                self.axes_phys_unit = False
                self.item = make.xyimage(np_array(list(range(len(x)))),np_array(list(range(len(y)))),data,title=title[ii])
                self.imagewidget.plot.set_titles(xlabel=prefs.X_LABEL_ARBITRARY, ylabel=prefs.Y_LABEL_ARBITRARY)
            self.item.interpolate=(0,)
            if title[ii] != 'L':
                self.item.hide()
