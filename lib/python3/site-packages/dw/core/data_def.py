...
# Data file Opening ###########################################################
    def open_data(self, file_name, file_mode, file_index, file_type=file_type_default):
        """Invoke the file open command
        file_name: path to the opening file
        file_type: file format (default: fyle_type_default)
        """
        dwcomm.OpenCommand(self, file_name, file_type, file_index, file_mode)
#        Invoker.execute(command, inspect.stack()[1][4][0][0:-1])
###############################################################################
    def close(self):
    ...
    def del_flagset(self, i_dataset, k_flagset, filelist):
        """Invoke the command to delete a flagging set
        i_dataset: index of the dataset table
        k_flagset:  key of the flagset in the dictionary
        """

        dwcomm.DelFlagSetCommand(self, i_dataset, k_flagset, filelist)
...
###############################################################################
    ...
    def flagsets2np_array(self, i_dataset):
        ...
        """
        flagsets = {}
        keys = list(self.datasets[i_dataset].flagsets.keys())
        for key in keys:
